...
export default function Chatbot() {
  const [messages, setMessages] = useState([
    { type: 'bot', content: 'Hello! How can I assist you with your farming needs today?' },
  ])
  ...
  const handleSend = () => {
    if (input.trim()) {
      ...
      setTimeout(() => {
        setMessages(prev => [...prev, { type: 'bot', content: "I'm processing your request. Please give me a moment." }])
      }, 1000)
    }
  }
    ...
    <>
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-3xl font-semibold text-neutral-800 mb-6">AI Chatbot</h1>
              ...
              {messages.map((message, index) => (
                <div key={index} className={`mb-4 ${message.type === 'user' ? 'text-right' : ''}`}>
                  <div className={`inline-block rounded-2xl p-3 ${
                    message.type === 'user' ? 'bg-green-500 text-white' : 'bg-neutral-100 text-neutral-800'
                  }`}>
                    <p>{message.content}</p>
                  </div>
                </div>
